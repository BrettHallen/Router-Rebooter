...
/* Well-known, reliable IPs to ping - choose one */
// Improvement: Randomly choose one from the list each time?
IPAddress targetIP(1, 1, 1, 1);            // Cloudflare primary
// IPAddress targetIP(1, 0, 0, 1);         // Cloudflare  secondary
// IPAddress targetIP(8, 8, 8, 8);         // Google primary
// IPAddress targetIP(8, 8, 4, 4);         // Google secondary
...
void setup() 
{
  ...
  Serial.println(">> Hardware initialised: Router ON, OK LED ON");
}
...
void loop() 
{
  ...
  while (true) {
    ...
    // 5-minute recovery countdown with flashing OK LED (step 6)
    Serial.print(">> Starting ");
    Serial.print(RECOVERY_DELAY);
    Serial.println("min recovery wait (OK LED flashing)");
